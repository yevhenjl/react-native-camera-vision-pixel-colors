///
/// PixelColorsResult.kt
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © Marc Rousavy @ Margelo
///

package com.margelo.nitro.cameravisionpixelcolors

import androidx.annotation.Keep
import com.facebook.proguard.annotations.DoNotStrip


/**
 * Represents the JavaScript object/struct "PixelColorsResult".
 */
@DoNotStrip
@Keep
data class PixelColorsResult(
  @DoNotStrip
  @Keep
  val uniqueColorCount: Double,
  @DoNotStrip
  @Keep
  val topColors: Array<RGBColor>,
  @DoNotStrip
  @Keep
  val brightestColors: Array<RGBColor>
) {
  /* primary constructor */

  companion object {
    /**
     * Constructor called from C++
     */
    @DoNotStrip
    @Keep
    @Suppress("unused")
    @JvmStatic
    private fun fromCpp(uniqueColorCount: Double, topColors: Array<RGBColor>, brightestColors: Array<RGBColor>): PixelColorsResult {
      return PixelColorsResult(uniqueColorCount, topColors, brightestColors)
    }
  }
}
